...
- `REACT_APP_BACKEND_URL=https://room-scan-validation.preview.emergentagent.com` (WRONG)
- Should be `https://room-scan-validation.preview.emergentagent.com` (CORRECT)
- API calls went to non-existent server → returned 404 HTML page

**Fix Applied:**
```bash
# Updated /app/frontend/.env
REACT_APP_BACKEND_URL=https://room-scan-validation.preview.emergentagent.com
```

**Verification:**
```bash
# Before fix (wrong URL)
curl -s https://room-scan-validation.preview.emergentagent.com/api/auth/login
→ "404 page not found"
