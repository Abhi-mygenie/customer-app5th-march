...
- 2026-02 — Phase 2 Room Guest Auto-Populate + Lock IMPLEMENTED. Files: `orderService.ts` (extended `checkTableStatus` to return `tableType` + `guest`), `LandingPage.jsx` (auto-fill name/phone, persist `guestCustomer.locked: true`, extend Phase 1 gate to also block on `RM + Not Available + incomplete userinfo`), `LandingCustomerCapture.jsx` (`readOnly` prop + lock helper), `CustomerDetails.jsx` (`readOnly` prop + lock helper), `ReviewOrder.jsx` (read `locked` flag → pass to `<CustomerDetails>`). Live API uses `"table type"` (with space) — read defensively alongside `table_type`. 12 unit tests passing (6 Phase 2 service + 6 Check-In transformer). All 5 baseline scenarios still pass (vacant block, 716 unaffected, table/walk-in/takeaway untouched). No payload / tax / KOT / socket / backend changes.
