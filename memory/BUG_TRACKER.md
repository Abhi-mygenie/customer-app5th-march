...
|--------|-------|----------|--------|------------|------------|----------|
| BUG-050 | CRM: Addresses not shared across restaurants — new restaurant gets blank addresses | 🟡 P1 | ⏳ CRM Team | Apr 14 | - | **CRM Backend Bug.** Customer addresses are stored per `user_id` (restaurant-scoped). When a customer registers on a new restaurant, they get 0 addresses even if they have addresses saved on other restaurants. Addresses are personal data (Home, Work) and should be reusable across restaurants. Needs CRM backend change to share addresses cross-restaurant by phone number. See BUG-050 detail below. |
| BUG-049 | Landing page: stale name persists when changing phone number | 🟡 P1 | ⏳ Open | Apr 14 | - | `nameAutoFilled` flag clears on any name field interaction (focus/click). After that, changing phone doesn't clear the stale name. `check-customer` returning `exists:false` for new phone also doesn't clear it. Fix: clear name when check-customer returns different/null name, regardless of flag. |
| BUG-048 | crmFetch Content-Type header dropped for authenticated POST requests | 🔴 P0 | ✅ Fixed | Apr 14 | Apr 14 | `...options` spread overwrote merged headers, dropping Content-Type. CRM received body as string not JSON → Pydantic model_attributes_type error. Fixed by destructuring headers before spread. |
...
- `REACT_APP_BACKEND_URL=https://customer-app-march-2.preview.emergentagent.com` (WRONG)
- Should be `https://customer-app-march-2.preview.emergentagent.com` (CORRECT)
- API calls went to non-existent server → returned 404 HTML page

**Fix Applied:**
```bash
# Updated /app/frontend/.env
REACT_APP_BACKEND_URL=https://customer-app-march-2.preview.emergentagent.com
```

**Verification:**
```bash
# Before fix (wrong URL)
curl -s https://customer-app-march-2.preview.emergentagent.com/api/auth/login
→ "404 page not found"
...
## BUG-050: CRM Backend — Addresses Not Shared Across Restaurants

**Priority:** P1 | **Status:** Pending CRM Team | **Date Found:** April 14, 2026  
**Component:** CRM Backend (`crm.mygenie.online`)  
**Reported By:** Customer App Team

### Problem Statement

When a customer who has saved addresses on one restaurant (e.g., restaurant 509 with 6 saved addresses) registers or logs into a **new restaurant** (e.g., restaurant 669), their delivery address page shows **0 addresses**. The customer has to re-enter all addresses from scratch for every new restaurant they visit.

### Current Behavior (Verified via DB + API)

Customer phone `7505242126`:

| Restaurant | user_id | Name | Addresses | Source |
|------------|---------|------|-----------|--------|
| 509 | pos_0001_restaurant_509 | Abhishek | **10 addresses** | POS synced |
| 478 | pos_0001_restaurant_478 | Abhishek | 0 | POS synced |
| 675 | pos_0001_restaurant_675 | Abhishek | 0 | POS synced |
| 669 | pos_0001_restaurant_669 | Nilanjana | 0 | Registered via app |

Customer phone `1234567890`:

| Restaurant | Addresses |
|------------|-----------|
| 509 | **6 addresses** (Home, Work with lat/lng) |
| 675 | 0 |
| 478 | 0 |

### Root Cause

- Each restaurant has a **separate customer document** keyed by `user_id: "pos_{posId}_restaurant_{restaurantId}"`
- Addresses are stored **inside** each restaurant's customer document (`addresses: []` array)
- CRM token is scoped to a specific `user_id` → `/customer/me/addresses` returns ONLY that restaurant's addresses
- When `register` or `verify-otp` creates a new customer record for a new restaurant, it starts with **empty addresses**
- No cross-restaurant address sharing or copying logic exists

### Expected Behavior

Addresses are personal data (Home, Work, etc.) tied to the **person**, not the restaurant. A customer's saved addresses should be available across all restaurants they visit. When a customer registers on a new restaurant:

1. Their existing addresses (from any restaurant) should be available on the delivery address page
2. New addresses added on any restaurant should be accessible from other restaurants
3. Restaurant-specific data (points, wallet, orders) should remain scoped to the restaurant

### Current Shared vs Scoped Data

| Data | Current Scoping | Should Be |
|------|----------------|-----------|
| Name | Shared (via POS sync) | Shared ✅ |
| Phone, Email | Shared | Shared ✅ |
| **Addresses** | **Per restaurant** | **Shared** ❌ |
| Points | Per restaurant | Per restaurant ✅ |
| Wallet | Per restaurant | Per restaurant ✅ |
| Orders | Per restaurant | Per restaurant ✅ |
| Password | Per restaurant | Debatable (could be shared) |

### Proposed Fix Options (CRM Team)

**Option A — Shared address store:**  
Create a separate `customer_addresses` collection keyed by phone number (not user_id). All restaurants read/write from this shared store.

**Option B — Copy on register:**  
When a new customer record is created for a restaurant, copy addresses from an existing record with the same phone number.

**Option C — Cross-restaurant lookup in API:**  
`/customer/me/addresses` endpoint falls back to checking other restaurant records for the same phone if current restaurant has no addresses.

### Impact

- Every customer visiting a new restaurant for delivery has to re-enter all their addresses
- Friction in the delivery flow → potential order abandonment
- Especially painful for customers visiting multiple restaurants in the same chain/city
