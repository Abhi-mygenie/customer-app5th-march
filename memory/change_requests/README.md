...
| [CR-2026-09-15-003](./CR-2026-09-15-003-canonical-phone-alignment/INTAKE_DOC.md) | Canonical phone alignment POS↔CRM↔App: `extractPhoneNumber` blanks non-`+91` numbers (order unlinkable), `stripPhonePrefix` keeps cc — from CRM INV-018 | 📝 REGISTERED — **BLOCKED** on POS Q6/Q7 + CRM P-8 decision | P2 / **CRITICAL** | 2 | POS answers; sequence with CR-009 | — CRM confirms `/scan/orders`, `/scan/loyalty`, `/scan/points/history`, `/scan/wallet/history` exist (live-verified). 4 CRM discrepancies (D1–D4) + 10 app-side gaps (G1–G10, incl. duplicate config source) in `CRM_REPLY_VALIDATION.md`. No code. | **P1** | 2 | OpenAPI received, UAT confirmed. §9 proposes CR-A (Profile v2 adapter), CR-B (skip-otp bypass), INV-2026-09-15-002 (config source of truth), 2 fold-ins to CR-007. **SHARED-DB fact recorded (§10)**: OD-2 moot; CR-006/CR-014 collection changes now cross-team CRITICAL. Awaiting OD-3..OD-7 → INTAKE |
