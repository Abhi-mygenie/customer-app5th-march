...
const config = {
  ...
  enableVisualEdits: false, // Disabled - visual edits plugin has compatibility issues
};
...
webpackConfig.devServer = (devServerConfig) => {
  // webpack-dev-server v5 compatibility: remove deprecated v4 lifecycle hooks.
  // Use delete to fully remove them (setting to undefined still fails schema validation).
  if ('onBeforeSetupMiddleware' in devServerConfig) {
    const legacyBefore = devServerConfig.onBeforeSetupMiddleware;
    delete devServerConfig.onBeforeSetupMiddleware;
    if (legacyBefore && typeof legacyBefore === 'function') {
  ...
  if ('onAfterSetupMiddleware' in devServerConfig) {
    ...
    if (legacyAfter && typeof legacyAfter === 'function') {
      const existingSetup = devServerConfig.setupMiddlewares;
      devServerConfig.setupMiddlewares = (middlewares, devServer) => {
